				...
				<Form>
					...
					<Dropdown v-if="filters.length" trigger="click" class="dropdown" @command="addConditions" @visible-change="onVisibleChange" placement="bottom-start">
						...
						<DropdownMenu slot="dropdown" class="dropdownMenu" v-if="filters.length">
							...
							<DropdownItem :command="item.code" v-for="item in filterList" :key="item.id">{{ item.name }}</DropdownItem>
						</DropdownMenu>
					</Dropdown>
	...
	computed: {
		...
		filterLink() {
			...
			if (index > -1) {
				...
				return `/${name}-${childName.replace(/\[.+\]/g, "")}`;
			}
		},
		curChain() {
			const [groupI, parent, child] = this.currentNodeKey.split("-");
	...
	methods: {
		...
		async getDetailShow() {
			const params = {
				...
				anchorEquipmentId: this.options.anchorEquipmentId,
				containerId: this.options.containerId,
			};
			const { result, data } = await setFilter.getDetailShow(params);
			if (result == "success") {
				const { relChainListList } = data;
