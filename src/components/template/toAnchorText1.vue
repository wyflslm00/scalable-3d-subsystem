		...
		<div class="cnt">
			...
			<div class="rht">
				<template v-if="JSON.stringify(connectEquipObject) != '{}'">
					<div class="title">
						<span class="name" @click="directAlert({ type: 0 })"> 编辑锚点 </span>
					</div>
					<div class="chartBox">
					...
					<!-- {{ curAnchors }} -->
					<to-direct :visible.sync="vis_direct" v-if="vis_direct" :directData="directData" @confirm="directConfirm" @close="directClose"></to-direct>
...
import ToDirect from "@/components/template/toDirect.vue";
	...
	methods: {
		...
		directAlert(data) {
			...
			if (data.type === 1) {
				//初始
				obj.type = 1;
				obj.classCodeFrom = "ACCCOT";
				obj.classCodeTo = "ACCCOP";
			} else {
				//回显
				obj = { code: "CoolingWaterLoop/Supply", direction: "1", name: "空调冷却水管网/空调冷机冷却水进水", pipeRiserLimited: false, pipeStepLimited: false };
				obj.type = 0;
				obj.classCodeFrom = "ACCCOT";
				obj.classCodeTo = "ACCCOP";
			}
			...
			this.directData = obj;
			this.vis_direct = true;
		},
		directConfirm(data) {
			console.error("返回值-" + data);
			this.directClose();
		},
		directClose() {
			this.vis_direct = false;
			this.directData = {};
		},
	},
	watch: {
